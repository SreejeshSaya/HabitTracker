...
export function makeStreakGraph(habit: any,history: Array<any>,maxDays: number=90){
   if (history.length==0){
      return []
   }
   ...
   for (let i=history.length-1;i>=1;i--){
      ...
      let curr = history[i]
      let range = linearInterpolate(prev.streak,curr.streak,daysDifference(removeTime(prev.date),removeTime(curr.date))+1)
   ...
   if (first.valueOf()!=created.valueOf()){ //if not completed on first day
      let range = linearInterpolate(0,history[0].streak,daysDifference(created,first)+1)
      points = [...range,...points]
   }
   else {
...
export function makePunctualityGraph(habit: any,history: Array<any>,maxDays: number=90){
   if (history.length==0){
      return []
   }
   let now = removeTime(new Date())
   ...
   for (let i=history.length-1;i>=1;i--){
      let prev = history[i-1]
      let curr = history[i]
      let range = linearInterpolate(prev.punctuality,curr.punctuality,daysDifference(removeTime(prev.date),removeTime(curr.date))+1)
      points = [...range.slice(1),...points]
      // console.log(range,daysDifference(removeTime(prev.date),removeTime(curr.date))+1,points,prev.date,curr.date)
   }
   ...
   // console.log(first,created)
   if (first.valueOf()!=created.valueOf()){ //if not completed on first day
      let range = linearInterpolate(0,history[0].punctuality,daysDifference(created,first)+1)
      points = [...range,...points]
   }
   else {
